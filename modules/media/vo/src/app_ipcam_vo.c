...
CVI_S32 app_ipcam_Vo_Start_Postproc(const APP_PARAM_VO_CFG_T* const pstVoCfg)
{
    ...
    APP_CHK_RET(CVI_VO_SetChnParam(
                    pstVoCfg->stDstChn.s32DevId,
                    pstVoCfg->stDstChn.s32ChnId,
                    &pstVoCfg->stChnParam),
                    "CVI_VO_SetChnParam");

#ifdef GDC_SUPPORT
    if (pstVoCfg->enRotation != ROTATION_0) {
        APP_CHK_RET(CVI_VO_SetChnRotation(
                        pstVoCfg->stDstChn.s32DevId,
                        pstVoCfg->stDstChn.s32ChnId,
                        pstVoCfg->enRotation),
    ...
    if (pstVoCfg->stBorderAttr.enable != 0) {
        APP_CHK_RET(CVI_VO_SetChnBorder(
                        pstVoCfg->stDstChn.s32DevId,
                        pstVoCfg->stDstChn.s32ChnId,
                        &pstVoCfg->stBorderAttr),
                        "CVI_VO_SetChnBorder");
    }

    if (pstVoCfg->enMirrorType != VO_CHN_MIRROR_NONE) {
        APP_CHK_RET(CVI_VO_SetChnMirror(
                        pstVoCfg->stDstChn.s32DevId,
                        pstVoCfg->stDstChn.s32ChnId,
                        pstVoCfg->enMirrorType),
                        "CVI_VO_GetChnMirror");
    }

    if (pstVoCfg->bZoom != 0) {
        APP_CHK_RET(CVI_VO_SetChnZoomInWindow(
                        pstVoCfg->stDstChn.s32DevId,
                        pstVoCfg->stDstChn.s32ChnId,
                        &pstVoCfg->stZoomAttr),
                        "CVI_VO_SetChnZoomInWindow");
    }
