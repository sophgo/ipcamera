 ...
 *                         G L O B A L    D A T A                         *
 **************************************************************************/
APP_PARAM_SYS_CFG_S g_stSysAttrCfg = {0};
APP_PARAM_SYS_CFG_S * g_pstSysAttrCfg = &g_stSysAttrCfg;
...
int app_ipcam_Sys_Init(void)
{
    ...
    if(pModuleCfg->alios_sys_mode){
        ret = CVI_SYS_Init();
        if(ret != CVI_SUCCESS){
            APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_SYS_Init failed with %#x\n", ret);
            return ret;
        }
    }else{
