            ...
            int nbr_mesh)
{
    ...
    s32Ret = CVI_GDC_UpdateMeshCoordinate(
                pstGdcCfg->LdcAttr.stAttr.stGridInfoAttr.gridBindName,
                src_x_mesh,
                src_y_mesh,
                dst_x_mesh,
                dst_y_mesh,
                nbr_mesh);
    if (s32Ret) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR,"CVI_GDC_UpdateMeshCoordinate failed!\n");
    }

    CVI_SYS_IonFlushCache(pstGdcCfg->stTask.au64privateData[0], (void *)pstGdcCfg->stTask.au64privateData[1], 0x150000);
        ...
        VIDEO_FRAME_INFO_S *pstVideoFrameOut)
{
    ...
    s32Ret = GDC_COMM_PrepareFrame(&size_out, enPixelFormat, pstVideoFrameOut);
    if (s32Ret) {
    ...
    memcpy(&pstgdccfg->stTask.stImgIn, pstVideoFrameIn, sizeof(pstgdccfg->stTask.stImgIn));
    memcpy(&pstgdccfg->stTask.stImgOut, pstVideoFrameOut, sizeof(pstgdccfg->stTask.stImgOut));
...
exit1:
    if (s32Ret != CVI_SUCCESS)
        if (pstgdccfg->hHandle)
            s32Ret |= CVI_GDC_CancelJob(pstgdccfg->hHandle);
    APP_PROF_LOG_PRINT(LEVEL_ERROR, "app_ipcam_Gdc_SendFrame fail!\n");
    return CVI_FAILURE;
}
...
CVI_S32 app_ipcam_Gdc_ReleaseFrame(APP_GDC_CFG_T *pstGdcCfg) {
    ...
    VB_BLK blk_in = VB_INVALID_HANDLE;
    VB_BLK blk_out = VB_INVALID_HANDLE;

    if (pstGdcCfg->stTask.stImgIn.stVFrame.u64PhyAddr[0]) {
        blk_in = CVI_VB_PhysAddr2Handle(pstGdcCfg->stTask.stImgIn.stVFrame.u64PhyAddr[0]);
        if(blk_in != VB_INVALID_HANDLE){
            s32Ret =  CVI_VB_ReleaseBlock(blk_in);
            if (s32Ret != CVI_SUCCESS) {
                APP_PROF_LOG_PRINT(LEVEL_ERROR,"CVI_VB_ReleaseBlock(blk in) failed!\n");
                return s32Ret;
            }
        }
    }

    if (pstGdcCfg->stTask.stImgOut.stVFrame.u64PhyAddr[0]) {
        blk_out = CVI_VB_PhysAddr2Handle(pstGdcCfg->stTask.stImgOut.stVFrame.u64PhyAddr[0]);
        if(blk_out != VB_INVALID_HANDLE){
            s32Ret =  CVI_VB_ReleaseBlock(blk_out);
            if (s32Ret != CVI_SUCCESS) {
                APP_PROF_LOG_PRINT(LEVEL_ERROR,"CVI_VB_ReleaseBlock(blk out) failed!\n");
                return s32Ret;
            }
        }
    }
...
CVI_S32 app_ipcam_Gdc_SendFile(APP_GDC_CFG_T *pstGdcCfg) {
    ...
    memset(&stOutVideoFrame, 0, sizeof(VIDEO_FRAME_INFO_S));
    ...
    s32Ret = app_ipcam_Gdc_SendFrame(pstGdcCfg, &stInVideoFrame, &stOutVideoFrame);
    if (s32Ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "app_ipcam_Gdc_SendFrame failed!\n");
        return s32Ret;
    }

    s32Ret = app_ipcam_Gdc_EndJob(pstGdcCfg);
    if (s32Ret != CVI_SUCCESS) {
