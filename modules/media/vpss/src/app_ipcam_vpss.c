    ...
    for (VPSS_CHN VpssChn = 0; VpssChn < VPSS_MAX_PHY_CHN_NUM; VpssChn++) {
        ...
        if (pstVpssGrpCfg->abChnEnable[VpssChn]) {
            ...
            for (int idx = 0; idx < pstSysCfg->u8SbmCnt; ++idx) {
                if ((VpssGrp == pstSysCfg->pstSbmCfg[idx].s32SbmGrp) && (VpssChn == pstSysCfg->pstSbmCfg[idx].s32SbmChn)) {
                    ...
                    stVpssChnBufWrap.u32BufLine = pstSysCfg->pstSbmCfg[idx].s32WrapBufLine;
                    stVpssChnBufWrap.u32WrapBufferSize = pstSysCfg->pstSbmCfg[idx].s32WrapBufSize;
