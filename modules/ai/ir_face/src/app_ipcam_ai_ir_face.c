...
static CVI_S32 app_ipcam_Ai_FD_init()
{
    ...
    s32Ret = TDL_OpenModel(g_AI_Handle, CVI_TDL_SUPPORTED_MODEL_SCRFDFACE, app_ipcam_Ai_IR_FD_Param_Get()->model_path_fd, 0);
    if (s32Ret != CVI_SUCCESS) {
    ...
    s32Ret = TDL_OpenModel(g_AI_Handle, TDL_SUPPORTED_MODEL_IRLIVENESS, app_ipcam_Ai_IR_FD_Param_Get()->model_path_fr, 0);
    if (s32Ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, " TDL_OpenModel err ret %d module_path: %s \r\n", s32Ret, app_ipcam_Ai_IR_FD_Param_Get()->model_path_fr);
    }
    //打开模型IRFR : ir_recogition.cvimodel
    s32Ret = TDL_OpenModel(g_AI_Handle, CVI_TDL_SUPPORTED_MODEL_FACERECOGNITION, app_ipcam_Ai_IR_FD_Param_Get()->model_path_fr, 0);
    if (s32Ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, " TDL_OpenModel err ret %d module_path: %s \r\n", s32Ret, app_ipcam_Ai_IR_FD_Param_Get()->model_path_fr);
    }
...
static CVI_S32 app_ipcam_Ai_IR_FD_Recognition(VIDEO_FRAME_INFO_S *pstFrame, cvtdl_face_t * pstFaceResult)
{
    ...
    ret = CVI_TDL_FaceDetection(g_AI_Handle, pstFrame, CVI_TDL_SUPPORTED_MODEL_SCRFDFACE, pstFaceResult);
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_TDL_FaceDetection failed ret %#x!\r\n", ret);
        return CVI_FAILURE;
    }
    if (pstFaceResult->size != 1) {
        //APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_TDL_FaceDetection err face_meta.size %d\r\n", pstFaceResult->size);
        return CVI_FAILURE;
    }
...
CVI_S32 app_ipcam_Ai_IR_FD_Register(char * gallery_name)
{
    ...
    CVI_TDL_FaceDetection(g_AI_Handle, &stFrame, CVI_TDL_SUPPORTED_MODEL_SCRFDFACE, &stFaceMeta);
