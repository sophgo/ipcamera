...
static CVI_VOID *Thread_Cry_PROC(CVI_VOID *pArgs)
{
    ...
    while (app_ipcam_Ai_Cry_ProcStatus_Get()) {
        ...
        s32Ret = TDL_Classfification(g_CryAiHandle, g_pstCryCfg->model_id, image_handle, &obj_info);  // Detect the audio
...
static CVI_S32 app_ipcam_Ai_Cry_Proc_Init(CVI_VOID)
{
    ...
    {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "TDL_CreateHandlehas created\n");
        return s32Ret;
    }
...
int app_ipcam_Ai_Cry_Stop(void)
{
    ...
    TDL_CloseModel(g_CryAiHandle, g_pstCryCfg->model_id);
