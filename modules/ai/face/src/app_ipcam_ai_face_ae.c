 ...
 **************************************************************************/
#if 1
static APP_PARAM_AI_FACE_AE_CFG_S g_stFaceAeCfg = {
    .bEnable = CVI_TRUE,
...
static APP_PARAM_AI_FACE_AE_CFG_S *g_pstFaceAeCfg = &g_stFaceAeCfg;
...
static CVI_U32 app_ipcam_Ai_FD_GetStatistics(VIDEO_FRAME_INFO_S *pstFrame, Coordinate_t tMax)
{
    ...
    gridPerSizeX = (pstViChnCfg->astChnInfo[0].u32Width / g_pstFaceAeCfg->AE_Grid_Column) ? (pstViChnCfg->astChnInfo[0].u32Width / g_pstFaceAeCfg->AE_Grid_Column) : 1;
    gridPerSizeY = (pstViChnCfg->astChnInfo[0].u32Height / g_pstFaceAeCfg->AE_Grid_Row) ? (pstViChnCfg->astChnInfo[0].u32Height / g_pstFaceAeCfg->AE_Grid_Row) : 1;
    ...
    for (i = rowStartIdx; i < rowEndIdx; i++)
    {
        ...
        {
            RValue = stAeStat.au16FEZoneAvg[0][0][i][j][g_pstFaceAeCfg->AE_Channel_R];
            BValue = stAeStat.au16FEZoneAvg[0][0][i][j][g_pstFaceAeCfg->AE_Channel_B];
            GValue = stAeStat.au16FEZoneAvg[0][0][i][j][g_pstFaceAeCfg->AE_Channel_GR] +
                stAeStat.au16FEZoneAvg[0][0][i][j][g_pstFaceAeCfg->AE_Channel_GB];
            GValue = GValue / 2;
    ...
    if (aeWinCount > g_pstFaceAeCfg->Face_AE_Min_Cnt)
    {
...
CVI_VOID app_ipcam_Ai_FD_AEStart(VIDEO_FRAME_INFO_S *pstFrame, cvtdl_face_t *pstFace)
{
    ...
    if (pstFace->size > 0) 
    {
        ...
        {
            if ((pstFace->info[i].bbox.score * 100) < g_pstFaceAeCfg->Face_Score_Threshold)//侦测框是人脸的概率
            {
                continue;
            }
            //选取最大人脸框
        ...
        if ((iValue > 0) && ((iValue >= g_pstFaceAeCfg->Face_target_Luma_H_range) || (iValue <= g_pstFaceAeCfg->Face_target_Luma_L_range))) 
        {
            memset(&stExpAttr, 0, sizeof(stExpAttr));
            CVI_ISP_GetExposureAttr(ViPipe, &stExpAttr);
            stExpAttr.stAuto.u16EVBias = (g_pstFaceAeCfg->Face_target_Luma * 1024) / iValue;
            if (stExpAttr.stAuto.u16EVBias < g_pstFaceAeCfg->Face_target_Evbias_L_range)
            {
                stExpAttr.stAuto.u16EVBias =  g_pstFaceAeCfg->Face_target_Evbias_L_range;
            }
            else if (stExpAttr.stAuto.u16EVBias >  g_pstFaceAeCfg->Face_target_Evbias_H_range)
            {
                stExpAttr.stAuto.u16EVBias =  g_pstFaceAeCfg->Face_target_Evbias_H_range;
            }
    ...
    {
        if (bSetFaceAe && (stFrameCount > g_pstFaceAeCfg->Face_ae_restore_time)) 
        {
            memset(&stExpAttr, 0, sizeof(stExpAttr));
            CVI_ISP_GetExposureAttr(ViPipe, &stExpAttr);
