...
static uint32_t g_lost_start_time;
...
static CVI_VOID *Thread_Object_Track_Proc(CVI_VOID *pArgs)
{
	...
	while (app_ipcam_Ai_Object_Track_ProcStatus_Get()) {
		...
		if (!track_init) {
			...
			read_size = fread(buf, 1, sizeof(buf), pFile);
			fclose(pFile);
			if (read_size == 0) {
			...
			if (search_type == TDL_FASTSAM) {
				if (g_pstObjTrackCfg->model_path_sam[0] == '\0') {
					APP_PROF_LOG_PRINT(LEVEL_WARN, "model_path_sam is empty, fallback to TDL_REJECT\n");
					search_type = TDL_REJECT;
				} else {
			...
			s32Ret = TDL_SetSingleObjectTracking(g_ObjectTrackTDLHandle, image, &empty_det_meta,
							     box, 4, g_frame_id, search_type, model_path);
			if (s32Ret != CVI_SUCCESS) {
			...
			if (track_meta.info != NULL) {
				...
				if (elapsed_time >= (uint32_t)(g_pstObjTrackCfg->lost_timeout_seconds * 1000)) {
