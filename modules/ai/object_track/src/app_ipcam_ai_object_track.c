    ...
    while (app_ipcam_Ai_Object_Track_ProcStatus_Get()) {
...

        {
            ...
            if (obj_meta.size > 0 &&
                ...
                APP_PROF_LOG_PRINT(LEVEL_TRACE, "Object detection: %d \n", obj_meta.size);
...
static CVI_VOID *Thread_Object_Track_Proc(CVI_VOID *pArgs)
{
    ...
    while (app_ipcam_Ai_Object_Track_ProcStatus_Get()) {
        ...
        if (!track_init) {
            ...
            s32Ret = TDL_SetSingleObjectTracking(g_ObjectTrackTDLHandle, image, &g_obj_meta, box, 1, 2);
            if (s32Ret != 0) {
            ...
            if (track_meta.info) {
                ...
                if (!g_lost_timer_started) {
                    ...
                    APP_PROF_LOG_PRINT(LEVEL_INFO, "enter elapsed_time = %d\n", elapsed_time);
