...
void *app_ipcam_OTA_UploadFW(void *param)
{
    ...
    {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%s][%d] %s is not R_OK.\n" , __func__ , __LINE__ , OTA_STATUS_LOG_DIR);
        if(mkdir(OTA_STATUS_LOG_DIR , 0755) == -1)
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%s][%d] create %s failed!\n" , __func__ , __LINE__ , OTA_STATUS_LOG_DIR);
    }
    s_otaStatusLog = fopen(OTA_STATUS_LOG_FILE, "w");
    if (s_otaStatusLog == NULL) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"fopen[%s]\n", OTA_STATUS_LOG_FILE);
        return NULL;
    }
...
static CgiParseResultType_E readHeadForZip(void)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"cmpHeadData = %s , HeadLength = %d\n", cmpHeadData, cmpHeadLength);
    int boffset = 0;
    int dcount = 0;
    ...
    while (1)
    {
        ...
        if (got != 1)
        {
            ...
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"mpRead return got = %d\n", got);
            return cgiParseIO;
        }
        if (dcount > 2000)
        {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"We matched zip header failed!\n");
            return  cgiParseIO;
        }
        dcount++;
        ...
        {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"We matched zip header.boffset = %d , zipHeadLength = %d\n", boffset, cmpHeadLength);
            boffset++;
...
static CgiParseResultType_E CgiParsePostMultipartInput()
{
    ...
    if (outf == NULL)
    {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"GetTempFile error!\n");
        return cgiParseIO;
    }
    else
    {
    ...
    while (readcnt < totalcnt)
    {
        ...
        if (readOnce < got)
        {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"the buffer is too long! (%d)\n", got - readOnce);
            fileBUF[readOnce] = '\0';
            fwrite(fileBUF, 1, readOnce, outf);
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"read %d bytes got %d bytes , total = %d\n", readcnt, readOnce, readcnt + readOnce);
        }
        else
        {
        ...
        readcnt += got;
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"read %d bytes got %d bytes\n", readcnt, got);
    }
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"readcnt = %d , totalcnt = %d\n", readcnt, totalcnt);
...
static CgiParseResultType_E GetTempFile(FILE **tFile)
{
    ...
    if(access(UPLOAD_FW_TEMP_DIR , R_OK) == -1)
    {
        ...
        {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%s][%d] mkdir %s failed!\n" , __func__ , __LINE__ , UPLOAD_FW_TEMP_DIR);
            return cgiParseIO;
        }
    }
    //check the tmp file
    if(access(tfileName , F_OK) == 0)
    {
        ...
        {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%s][%d] remove %s failed!\n" , __func__ , __LINE__ , tfileName);
            return cgiParseIO;
        }
    }
    /* Fix the permissions */
    ...
    {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%s][%d] creat %s failed!\n" , __func__ , __LINE__ , tfileName);
        return cgiParseIO;
    }
#else
...
void *app_ipcam_OTA_UnpackAndUpdate(void *param)
{
    ...
    if (tmpFile)
    {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%d] remove tmp file...\n" , __LINE__);
        remove(tmpFile);
    }
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%d] reboot the board...\n" , __LINE__);
    goto END;
FAIL:
