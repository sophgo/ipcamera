...
static int cleanOTADir(void)
{
    ...
    ret = system(cmd);
    if (ret != 0)
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s failed!ret = %d\n", cmd, ret);
...
static int getMtdInfo(otaUpdateInfo_t *mtdInfo)
{
    ...
    {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"popen mtd info failed!\n");
        return mtdNum;
    }
...
int app_ipcam_OTA_UpdataFW(FILE *otaLogFile)
{
    ...
    for (mtdIndex = 0; mtdIndex < mtdNum; mtdIndex++)
    {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"open dev name(%s).\n", mtdPartPath);

        mtdFd = open(mtdPartPath, O_RDWR);
        if (mtdFd <= 0)
        {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"open failed!\n");
        }

        if (ioctl(mtdFd, MEMGETINFO, &mtdInfo[mtdIndex]))
        {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"Can't get \"%s\" information.\n", mtdPartPath);
            close(mtdFd);
            continue;
        }
        ...
        {
            //打印分区信息
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"########## mtdInfo : %s #########\n" , updateInfo[mtdIndex].mtdName);
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"type = %d \n", mtdInfo[mtdIndex].type);
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"flags = %d \n", mtdInfo[mtdIndex].flags);
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"Size = %d \n", mtdInfo[mtdIndex].size);
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"eraseSize = %d\n", mtdInfo[mtdIndex].erasesize);
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"writeSize = %d\n", mtdInfo[mtdIndex].writesize);
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"OobSize = %d\n", mtdInfo[mtdIndex].oobsize);
            //如果文件存在，则继续
            if (access(updateInfo[mtdIndex].fileName, F_OK) == 0)
            {
                ...
                if (imageFd <= 0)
                {
                    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"open %s failed.\n", updateInfo[mtdIndex].fileName);
                    close(mtdFd);
                    return OTA_RET_FAIL;
                }
                ...
                if (updateInfo[mtdIndex].mtdNum != MTD_FIPBIN_NUM)
                {
                    ...
                    {
                        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"read %zd failed.ret = %d\n", sizeof(imageHead), len);
                        close(imageFd);
                        return OTA_RET_FAIL;
                    }

                    //打印image头信息
                    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"Magic = %d , Version = %d , ChunkHeaderSize = %d , TotalChunk = %d , DataLen = %d\n" ,
                    ...
                    {
                        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"read fileHead %zd failed. ret = %d", sizeof(fileHead), len);
                        close(imageFd);
                        return OTA_RET_FAIL;
                    }

                    //打印file烧录文件的头信息
                    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"ChunkType = %d , DataSize = %d , ProgramOffset = %d , Crc32 = %d\n" ,
                ...
                APP_PROF_LOG_PRINT(LEVEL_DEBUG,"erase start 0x%x , length 0x%x ...\n", eraseInfo.start, eraseInfo.length);
                if (ioctl(mtdFd, MEMERASE, &eraseInfo))
                {
                    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"Can't erase \"%s\" length = %d.\n", mtdPartPath, eraseInfo.length);
                    close(mtdFd);
                    return OTA_RET_FAIL;
                }
                APP_PROF_LOG_PRINT(LEVEL_DEBUG,"erase mtd%d success.\n", updateInfo[mtdIndex].mtdNum);
                ...
                while (1)
                {
                    ...
                    if (len <= 0)
                    {
                        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"read end ! ret = %d\n", len);
                        break;
                    }
                    ...
                    if (ret <= 0)
                    {
                        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"write failed! ret = %d\n", ret);
                        break;
                    }
                }
                close(imageFd);
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%d] delete backup fw...\n" , __LINE__);
    DeleteBackupFW();

    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[%d] fflush log... \n" , __LINE__);
