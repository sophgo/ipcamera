...
static int cvi_cgi_cmd_proc(httpd_conn *hc, char *cgi, char *cmd)
{
    ...
    if (i >= CGI_REG_MAX || flag_cmd != 1) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"can not find the cgi command[%s]\n", cmd);
    ...
    if (pFunCgiCmdProc != NULL) {
        ...
        if (ret != 0) {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"pFunCgiCmdProc fail \n");
            ret = -1;
            goto exit;
        }
    } else {
...
static int cvi_cgi_handle(httpd_conn *hc)
{
    ...
    if (hc == NULL) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"pstRequest & pstReponse can not be null\n\n\n");
        return -1;
    }

    if (hc->origfilename == NULL) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"origfilename can not be null\n\n\n");
        return -1;
    }
    ...
    if ((pCommand != NULL) && (strlen(pCommand) < MAX_CGI_INTER)) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"the command is invalide:\n%s\n", hc->origfilename);
        return -1;
    }
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"pCommand is: %s\n", pCommand);
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"szCommand is %s\n", szCommand);
...
int CVI_NET_Deinit()
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"exit: %s\n", __func__);
    return 0;
}
...
int CVI_NET_RegisterCgiCmd(CGI_CMD_S *cgi_cmd)
{
    ...
    if (i >= CGI_REG_MAX) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"command list is full\n");
        return -1;
    }
    pthread_mutex_unlock(&g_cgi_mutex);
