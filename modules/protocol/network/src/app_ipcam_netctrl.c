 ...
 **************************************************************************/
#if (RECORD_SUPPORT) || (AUDIO_SUPPORT)
void NetString2Str(const char *val, char *dstStr)
{
    char *pValue = NULL;
    pValue = strrchr(val, '=');
...
{
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"enter: %s ", __func__);

    int ret = 0;
    CGI_CMD_S stCgiCmd;

    if (cmd != NULL) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s %zu  ", cmd, strlen(cmd)+1);
        snprintf(stCgiCmd.cgi, strlen(cmd)+1, cmd);
    } else {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"error, cmd is NULL\n");
        ret = -1;
        goto EXIT;
    }

    if (val != NULL) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s %zu", val, strlen(val)+1);
...
static int NetString2Int(const char *val)
{
    ...
    value = atoi(array);
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s---int: %d\n", val, value);
    return value;
}
...
static int app_ipcam_LocalIP_Get(const char* adapterName, char* ipAddr)
{
    ...
    if (skfd < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"socket failed! strerror : %s\n", strerror(errno));
        return -1;
    }
    ...
    if (ioctl(skfd, SIOCGIFADDR, &ifr) == 0) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"ioctl failed! strerror : %s\n", strerror(errno));
        close(skfd);
        return -1;
    }
    ...
    // if (g_wsi == NULL) {
        if (app_ipcam_LocalIP_Get("eth0", ip) != 0) {
            if (app_ipcam_LocalIP_Get("wlan0", ip) != 0)
                APP_PROF_LOG_PRINT(LEVEL_DEBUG,"Error: [%s][%d] getlocalIP [eth0/wlan0] failed!\n",
                    __func__, __LINE__);
        }
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"getlocalIP [eth0/wlan0] %s\n", ip);
    // }
...
static int TakePhotoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_INFO, "===========TakePhotoCallBack=====================\n");
...
{
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"enter: %s, AmpType:%d value:%d\n", __func__, AmpType, value);
...
static int ImagePage_Set_Sharpness(int value)
{
    ...
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_ISP_GetSharpenAttr failed\n");
        return -1;
    }
...
{
    CVI_S32 ret = CVI_SUCCESS;
    ISP_TNR_ATTR_S stNioseTnrAttr;
    memset(&stNioseTnrAttr, 0, sizeof(ISP_TNR_ATTR_S));
    s_noise3d = MAX_VALUE - value;
    APP_PARAM_VI_CTX_S *pstViParamCfg = app_ipcam_Vi_Param_Get();
    int viPipe = pstViParamCfg->astPipeInfo[0].aPipe[0];
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"enter: %s, %d\n", __func__, value);
    ret = CVI_ISP_GetTNRAttr(viPipe, &stNioseTnrAttr);
    if (ret != CVI_SUCCESS) {
...
static int ImagePage_Set_Shutter_Enable(int value)
{
    ...
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"CVI_ISP_SetExposureAttr failed\n");
    }
...
static int ImagePage_Set_Shutter(int value)
{
    ...
    if (s_shutterEnabled == CVI_TRUE) {
        ...
        if (ret != CVI_SUCCESS) {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"CVI_ISP_GetExposureAttr failed\n");
        }
    } else {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"shutter MANUAL hasen't enabled\n");
    }
    return 0;
}
...
static int ImagePage_Set_Frequency(int value)
{
    ...
    if(s_antiflash){
        ...
        ret |= CVI_ISP_GetExposureAttr(viPipe, &stExpAttr);
        if (ret != 0){
            APP_PROF_LOG_PRINT(LEVEL_ERROR, "ImagePage_Set_Frequency failed\n");
        }
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "stExpAttr.stAuto.stAntiflicker.enFrequency = %d\n", stExpAttr.stAuto.stAntiflicker.enFrequency);
    } else{
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "[ ERROR ] please open 强闪开关 first!\n");
    }
    return 0;
}
...
static int ImagePage_Get_Rotation(APP_ROTATION_E eRType)
{
    ...
    if (s32Ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"CVI_VI_GetChnFlipMirror failed\n");
        return -1;
    }
...
static int SetImgInfoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    if (strstr(val, "brightness") != NULL) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s %d: error no support setting, %s %s\n", __func__, __LINE__, cmd, val);
    }
...
static int GetImgInfoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    if (ret != 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "get image info failed\n");
...
static int app_ipcam_VencAttr_Get(APP_VENC_ATTR_INFO_S stVencAttrInfo[])
{
    ...
    for (VencChn = 0; VencChn < APP_VENC_CHN_NUM; VencChn++) {
        ...
        if (CVI_VENC_GetChnAttr(VencChn, &stChnAttr)) {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"CVI_VENC_GetChnAttr failed\n");
            return -1;
        }
...
static int GetStreamCfgCallBack(void *param, const char *cmd, const char *val)
{
    ...
    if (str) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"cJson: %s\n", str);
        CVI_NET_AddCgiResponse(param, "%s", str);
        cJSON_free(str);
    }
...
static int app_ipcam_VencAttr_Set(APP_VENC_ATTR_INFO_S info[])
{
    ...
    for(int i = 0; i < APP_VENC_CHN_NUM; i++) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"enVencChn=0x%x\n", enVencChn);
    }
        ...
        if (stResetVencFlag[i].bProfile) {
            if(pstVencChnCfg->enType == PT_H264) {
                ...
                case APP_VIDEOPROFILE_HIGH:
                    ...
                    break;
                default:
                    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s cmd failed profile err %d\n", __func__, info[i].profile);
                    break;
                }
            }
        }
...
static int SetStreamCfgCallBack(void *param, const char *cmd, const char *val)
{
    ...
    // APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s\n", decode);

    cJsonRoot = cJSON_Parse(decode);
    if (cJsonRoot == NULL) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"parse fail.\n");
        return -1;
    }
    ...
    if (app_ipcam_VencAttrChange_Check(info)) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"all video channel attr not change!\n");
    }
    CVI_NET_AddCgiResponse(param, "ret: %d\n", ret);
    cJSON_Delete(cJsonRoot);
    return ret;
...
static int GetAiInfoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"enter: %s\n", __func__);
    #ifdef MD_SUPPORT
    // md
...
static int GetOsdInfoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"osd_text1_color = %d\n", color);
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"osd_text2color = %d\n", color);
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"osd_text3_color = %d\n", color);
...
static int SetOsdInfoCallBack(void *param, const char *cmd, const char *val)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%zu %zu\n", strlen(cmd), strlen(val));
    UrlDecode(val, decode);
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%s\n", decode);

    cjsonParser = cJSON_Parse(decode);
    if(cjsonParser == NULL) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "parse fail.\n");
        return -1;
    }
    APP_PARAM_OSDC_CFG_S *pstOsdcCfg = app_ipcam_Osdc_Param_Get();
    APP_OSDC_OBJS_INFO_S *pstOsdcPrivacy = app_ipcam_OsdcPrivacy_Param_Get();
