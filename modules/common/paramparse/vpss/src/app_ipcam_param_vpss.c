...
int Load_Param_Vpss(const char *file)
{
    ...
    for (grp_idx = 0; grp_idx < Vpss->u32GrpCnt; grp_idx++) {
        ...
        for (chn_idx = 0; chn_idx < grp_chn_cnt; chn_idx++) {
            ...
            Vpss->astVpssGrpCfg[grp_idx].aGdcEn[chn_idx] = ini_getl(tmp_section, "gdc_en", 0, file);
            if (Vpss->astVpssGrpCfg[grp_idx].aGdcEn[chn_idx]) {
                Vpss->astVpssGrpCfg[grp_idx].aGdcConfigId[chn_idx] = ini_getl(tmp_section, "gdc_config_id", 0, file);
            }

            Vpss->astVpssGrpCfg[grp_idx].stVpssChnYRatioEn[chn_idx] = ini_getl(tmp_section, "y_ratio_en", 0, file);
            if (Vpss->astVpssGrpCfg[grp_idx].stVpssChnYRatioEn[chn_idx]) {
            ...
            "GDC Enable=%d GDC Config=%d\n"
                ...
                , pastVpssChnAttr->u32Width
                , pastVpssChnAttr->u32Height
                , pastVpssChnAttr->u32Depth
                , pastVpssChnAttr->bMirror
                , pastVpssChnAttr->bFlip
                , pastVpssChnAttr->enVideoFormat
