...
int Load_Param_Osdc(const char *file)
{
    ...
    if (Osdc->enable) {
        for (j = 0; j < OSDC_NUM_MAX; j++) {
            ...
            APP_PROF_LOG_PRINT(LEVEL_INFO, "handle=%d bShow=%d Format=0x%x cpsSize=%d ModeId=%d DevId=%d ChnId=%d PdRect=%d MdRect=%d HdRect=%d CountRect=%d FdRect=%d TrackRect=%d HumanKeypointRect=%d osdcObjNum=%d\n",
                Osdc->handle[j], Osdc->bShow[j], Osdc->format[j], Osdc->CompressedSize[j], Osdc->mmfChn[j].enModId,
                Osdc->mmfChn[j].s32DevId, Osdc->mmfChn[j].s32ChnId, Osdc->bShowPdRect[j], Osdc->bShowMdRect[j], Osdc->bShowHdRect[j], Osdc->bShowCountRect[j], Osdc->bShowFdRect[j], Osdc->bShowTrackRect[j], Osdc->bShowHumanKeypointRect[j], Osdc->osdcObjNum[j]);
