...
const char *venc_mirror_type[MIRDIR_TYPE_BUTT] = {
    [MIRDIR_TYPE_NONE] = "MIRDIR_TYPE_NONE",
    [MIRDIR_TYPE_VER] = "MIRDIR_TYPE_VER",
    [MIRDIR_TYPE_HOR] = "MIRDIR_TYPE_HOR",
    [MIRDIR_TYPE_HOR_VER] = "MIRDIR_TYPE_HOR_VER"
};
...
int Load_Param_Venc(const char *file)
{
    ...
    for (i = 0; i < chn_num; i++) {
        ...
        if (ret != CVI_SUCCESS) {
            ...
            Venc->astVencChnCfg[i].enRotation = enum_num;
        }

        ini_gets(tmp_section, "mirror_type", " ", str_name, PARAM_STRING_NAME_LEN, file);
        ret = app_ipcam_Param_Convert_StrName_to_EnumNum(str_name, venc_mirror_type, MIRDIR_TYPE_BUTT, &enum_num);
        if (ret != CVI_SUCCESS) {
            ...
            Venc->astVencChnCfg[i].enMirrorDirextion = enum_num;
        }
        ...
        if (ret != CVI_SUCCESS) {
            APP_PROF_LOG_PRINT(LEVEL_INFO, "[%s][pixel_format] Fail to convert string name [%s] to enum number!\n", tmp_section, str_name);
        } else {
            APP_PROF_LOG_PRINT(LEVEL_INFO, "[%s][pixel_format] Convert string name [%s] to enum number [%d].\n", tmp_section, str_name, enum_num);
            Venc->astVencChnCfg[i].enPixelFormat = enum_num;
        }

        if (Venc->astVencChnCfg[i].enType == PT_H264 || Venc->astVencChnCfg[i].enType == PT_H265) {
            ...
            Venc->astVencChnCfg[i].u32MCUPerECS = ini_getl(tmp_section, "MCUPerECS", 0, file);
            APP_PROF_LOG_PRINT(LEVEL_INFO, "Venc_chn[%d] quality=%d\n", i, Venc->astVencChnCfg[i].u32quality);
        } else if (Venc->astVencChnCfg[i].enType == PT_MJPEG) {
            Venc->astVencChnCfg[i].u32MCUPerECS = ini_getl(tmp_section, "MCUPerECS", 0, file);
            APP_PROF_LOG_PRINT(LEVEL_INFO, "Venc_chn[%d] MCUPerECS=%d\n", i, Venc->astVencChnCfg[i].u32MCUPerECS);
        }
    }
