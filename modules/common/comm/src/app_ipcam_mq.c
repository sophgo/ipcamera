...
static int app_ipcam_MqSocket_Create(CVI_MQ_ENDPOINT_HANDLE_t pEpHdl)
{
    ...
    if (socket_fd < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] create socket fail, ret = %d, errno = %d\n", socket_fd, errno);
    ...
    if (ret < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] bind socket fail, ret = %d, errno = %d, addr = %s",
                ret, errno, un.sun_path);
        close(socket_fd);
        return CVI_MQ_ERR_FAILURE;
    }
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] Socket bind to %s\n", un.sun_path);

    FD_SET(socket_fd, &pEpHdl->fds);
    if (socket_fd > pEpHdl->fd_max) {
        pEpHdl->fd_max = socket_fd;
    }
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] listen local socket: %d\n", socket_fd);
...
static void mq_recv_worker(void *arg)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] Recv Task: start id=0x%x\n", pEpHdl->config.id);
    ...
    while (pEpHdl->exit != true) {
        ...
        if (ret > 0) {
            //APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] Recv Task: got input on socket %d\n", pEpHdl->socket_fd);
            ...
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] RX %zd bytes\n", recv_len);
            ...
            if (ret != CVI_MQ_SUCCESS) {
                APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] recv cb return err %d\n", ret);
                break;
            }
        } else if (ret < 0) {
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] Recv Task: exit id=0x%x\n", pEpHdl->config.id);
}
    ...
    CVI_MQ_ENDPOINT_HANDLE_t      *pEpHdl)
{
    ...
    if (pEpHandle == NULL) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ]: create pEpHdl alloc fail, errno = %d\n", errno);
    ...
    if (config->recv_cb) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] pEpHdl id=0x%x for TX only\n", config->id);
    }
    ...
    CVI_MQ_MSG_t                  *msg)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] TX %d bytes, to 0x%x\n", msg->len, msg->target_id);
    ret = sendto(socket_fd, (void*)msg, msg->len,
            MSG_DONTWAIT, (struct sockaddr *)&un, addr_len);
    if (ret < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] Send message fail, ret = %d, errno = %d, addr = %s",
                ret, errno, un.sun_path);
        if (errno == EAGAIN) {
            rc = CVI_MQ_ERR_AGAIN;
        } else {
...
int CVI_MQ_Send_Ack(CVI_MQ_ENDPOINT_HANDLE_t pEpHdl,
    ...
    if (ret < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] Send Ack fail, ret = %d, errno = %d",
                ret, errno);
        if (errno == EAGAIN) {
    ...
    int16_t                       recv_timeout)
{
    ...
    if(ret < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] bind fail, ret = %d, errno = %d, addr = %s",
                ret, errno, un.sun_path);
    }
    memset(&un, 0, sizeof(un));
    un.sun_family = AF_LOCAL;
    ...
    if(ret > 0) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ] Ack %d bytes\n", recv_len);
    } else if (ret < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ[ Select fail! ret = %d errno = %d\n", ret, errno);
        msg_ack->status = ret;
    } else {
        msg_ack->status = -110;
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"[MQ[ Ack Timeout !\n");
    }
