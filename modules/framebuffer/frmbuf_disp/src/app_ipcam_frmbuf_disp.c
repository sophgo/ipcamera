...
APP_PARAM_FRMBUF_DISP_CTX_S g_stDispFrmBufCtx, *g_pstDispFrmBufCtx = &g_stDispFrmBufCtx;

APP_PARAM_FRMBUF_DISP_CTX_S *app_ipcam_FrmBuf_Disp_Param_Get() {
    return g_pstDispFrmBufCtx;
}

static CVI_S32 app_ipcam_FrmBuf_Disp_DrawChar(CVI_VOID *arg, CVI_BOOL bgColor)
{
...
CVI_VOID *dispFrmBufThr(CVI_VOID *arg)
{
    ...
    APP_PARAM_FRMBUF_DISP_CTX_S *pstDispFrmBufCtx = NULL;
    ...
    pstDispFrmBufCtx = (APP_PARAM_FRMBUF_DISP_CTX_S *)arg;
    ...
    while (pstDispFrmBufCtx->thrFlag) {
        ...
        if ((bVdecSoftEnable == CVI_TRUE) 
            ...
            s32Ret = app_ipcam_FrmBuf_Disp_DrawChar((CVI_VOID *)pstFrmBufCtx, CVI_FALSE);
            if (s32Ret != CVI_SUCCESS) {
                APP_PROF_LOG_PRINT(LEVEL_ERROR, "draw char failed. s32Ret:%d. \n", s32Ret);
                continue;
            }
            ...
            s32Ret = app_ipcam_FrmBuf_Disp_DrawChar((CVI_VOID *)pstFrmBufCtx, CVI_TRUE);
            if (s32Ret != CVI_SUCCESS) {
                APP_PROF_LOG_PRINT(LEVEL_ERROR, "draw char failed. s32Ret:%d. \n", s32Ret);
                continue;
            }
...
CVI_S32 app_ipcam_FrmBuf_Disp_Start(CVI_VOID)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_INFO, "app_ipcam_FrmBuf_Disp_Start done.\n");
...
CVI_S32 app_ipcam_FrmBuf_Disp_Stop(CVI_VOID)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_INFO, "app_ipcam_FrmBuf_Disp_Stop done.\n");
