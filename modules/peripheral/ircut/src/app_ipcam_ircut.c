    ...
    if(g_bIRCut_Running) {
        if (g_stIRCutCtrl.bManualCtrl == CVI_TRUE) {
            ...
            if (state == IR_CUT_SWITCH_TO_NORMAL) {
                ...
                /* load PQ parameter */
                #ifdef ANONMSG_ENABLE
                for (CVI_U32 i = 0; i < pstViCtx->u32WorkSnsCnt; i++) {
                    app_ipcam_MsgAnonSendParam(ANONMSG_PQPARM, (pstViCtx->astPipeInfo[i].aPipe[0] << 16) | 1);
                }
                #endif
                ...
                #ifdef ANONMSG_ENABLE
                for (CVI_U32 i = 0; i < pstViCtx->u32WorkSnsCnt; i++) {
                    app_ipcam_MsgAnonSendParam(ANONMSG_PQPARM, (pstViCtx->astPipeInfo[i].aPipe[0] << 16) | 0);
                }
                #endif
                /* close ir filter*/
        ...
        APP_PROF_LOG_PRINT(LEVEL_ERROR,"IR_Cut Create failed!!!\n");
    }
}
...
int app_ipcam_IrCut_DeInit(void)
{
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"state detect exit\n");
