...
#define CHECK_GPIO_NUMBER(x)                                                                                       \
        do {                                                                                                       \
            if ((x < CVI_GPIO_MIN) || (x > CVI_GPIO_MAX)) {                                                        \
                APP_PROF_LOG_PRINT(LEVEL_DEBUG,"\033[0;31m GPIO %d is invalid at %s: LINE: %d!\033[0;39m\n", x, __func__, __LINE__);       \
                return -1;                                                                                         \
            }                                                                                                      \
        } while (0)
...
static int GpioExport(unsigned int gpio)
{
    ...
    if (fd < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio %d export error\n", gpio);
        return fd;
    }

    len = snprintf(buf, sizeof(buf), "%d", gpio);
    if (-1 == write(fd, buf, len)) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"write gpio export failed\n");
    }
    close(fd);
...
static int GpioUnexport(unsigned int gpio)
{
    ...
    if (fd < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio %d unexport error\n", gpio);
        return fd;
    }

    len = snprintf(buf, sizeof(buf), "%d", gpio);
    if (-1 == write(fd, buf, len)) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"write gpio unexport failed\n");
    }
    close(fd);
...
static int GpioDirectionSet(unsigned int gpio, CVI_GPIO_DIRECTION_E dir)
{
    ...
    if (fd < 0) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio %d open error\n", gpio);
        return fd;
    }

    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"mark %d , %s \n", dir, buf);
    if (dir) {
        if (-1 == write(fd, "out", 4)) {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"write gpio dir out failed\n");
        }
    } else {
        if (-1 == write(fd, "in", 3)) {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"write gpio dir in failed\n");
        }
    }
...
static int GpioDirectionGet(unsigned int gpio, CVI_GPIO_DIRECTION_E *dir)
{
    ...
    snprintf(buf, sizeof(buf), SYSFS_GPIO_DIR"/gpio%d/direction", gpio);
    if(access(buf, F_OK) == -1) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio_%d not export!\n", gpio);
        return -1;
        //GpioExport(gpio);
    }
    ...
    if (-1 == read(fd, &ch, 1)) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"read gpio dir failed\n");
    }
    if (ch == 'i') {
        *dir = CVI_GPIO_DIR_IN;
    } else if (ch == 'o') {
        *dir = CVI_GPIO_DIR_OUT;
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"get dir fail!\n");
        return -1;
    }
...
static int GpioValueGet(unsigned int gpio, unsigned int *value)
{
    ...
    if (-1 == read(fd, &ch, 1)) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"read gpio value failed\n");
    }
    //APP_PROF_LOG_PRINT(LEVEL_DEBUG," GpioValueGet = %c \n", ch);
...
static int GpioEdgeSet(unsigned int gpio, unsigned int edge_flag)
{
    ...
    if (w_cnt == -1) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"write gpio edge failed\n");
    }
    close(fd);
    return 0;
}
...
static int GpioPoll(unsigned int gpio, unsigned int event, FunType Fp)
{
    ...
    if (lseek(fd, 0, SEEK_SET) == -1) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio %d lseek error\n", gpio);
        return -1;
    }
    if (read(fd, &value, 1) == -1) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio %d read error\n", gpio);
        return -1;
    }
    ...
    while (1) {
        if (poll(&pfd, 1, 1000) == -1) {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio %d poll error\n", gpio);
            return -1;
        }
        ...
        if (pfd.revents & POLLPRI) {
            ...
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"poll value:%c\n", value);
            if (Fp) {
                Fp();
            }
        }
        if (pfd.revents & POLLERR) {
            APP_PROF_LOG_PRINT(LEVEL_DEBUG,"gpio %d POLLERR\n", gpio);
        }
