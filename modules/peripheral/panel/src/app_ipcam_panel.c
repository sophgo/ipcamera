...
CVI_S32 app_ipcam_Panel_PanelDesc_Get(
    const PANEL_NAME_E* const pstPanelName,
    const CVI_S32* const ps32DevNo,
    PANEL_DESC_T* const pstPanelDesc)
{
    _NULL_POINTER_CHECK_(pstPanelName, CVI_FAILURE);
    ...
    switch(*pstPanelName) {
        case PANEL_DSI_3AML069LP01G:
            ...
            pstPanelDesc->enPanelType = PANEL_MODE_DSI;
    ...
    if (pstPanelDesc->enPanelType == PANEL_MODE_DSI) {
        pstPanelDesc->pstDevCfg->devno = *ps32DevNo;
    }
